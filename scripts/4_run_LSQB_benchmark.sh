...
for QUERY_ID in ${QUERY_IDS[@]}; do
  for ((i=1; i<=REPEAT_TIMES; i++)); do
    ...
    output=$(timeout $TIMEOUT ./build/cuMatch ${LSQB_LGF_NAME} ${LGF_PATH} ${query} ${host_buffer_size} ${device_buffer_size} ${table_join_flag} ${in_memory_mode} ${gpu_streaming_mode_flag} || echo "timeout")
