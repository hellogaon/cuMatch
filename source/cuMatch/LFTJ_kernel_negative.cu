...
uint32_t set_intersection_initial_negative(
        uint32_t const& dep,
...
GSTREAM_DEVICE_ONLY ASH_FORCEINLINE
uint32_t set_intersection_resume_negative(
        uint32_t const& dep,
        uint32_t const& match_vid,
        uint32_t const& num_join_shards,
...
GSTREAM_CUDA_KERNEL
void LFTJ_kernel_negative(kernel_args const args) {
    ...
    for (uint32_t st_idx = 0; st_idx < kbuf->num_merged_subtasks; st_idx++) {
        ...
        while (true) {
            ...
            if (dep == num_query_vertices - 1) {
                uint32_t const pivot_shard_offset = set_intersection_initial_negative(
                    dep,
                    num_query_vertices,
                    num_query_edges, 
                    match_vid,
                ...
                if (is_finish[dep]) { 
                    if (state[dep] == state_type::INITIAL) {
                        ...
                        pivot_shard_offset = set_intersection_initial_negative(
                            dep,
                            num_query_vertices,
                            num_query_edges, 
                            match_vid,
                        ...
                        set_intersection_resume_negative(
                            dep,
                            match_vid,
                            num_join_shards,
                            pivot_shard_offset,
