...
uint32_t set_intersection_initial_constraint(
        uint32_t const& dep,
...
GSTREAM_DEVICE_ONLY ASH_FORCEINLINE
uint32_t set_intersection_resume_constraint(
        uint32_t const& dep,
        uint32_t const& match_vid,
        uint32_t const& num_join_shards,
        uint32_t const& num_constraints_per_block, // for constraint
...
GSTREAM_CUDA_KERNEL
void LFTJ_kernel_constraint(kernel_args const args) {
    ...
    for (uint32_t i = threadIdx.x; i < num_query_vertices; i += blockDim.x) {
        num_join_shards_arr[i] = prefix_sum_degree[i + 1] - prefix_sum_degree[i];
    ...
    for (uint32_t st_idx = 0; st_idx < kbuf->num_merged_subtasks; st_idx++) {
        ...
        while (true) {
            ...
            if (dep == num_query_vertices - 1) {
                uint32_t const pivot_shard_offset = set_intersection_initial_constraint(
                    dep,
                    num_query_vertices,
                    num_query_edges, 
                    match_vid,
                ...
                if (is_finish[dep]) { 
                    if (state[dep] == state_type::INITIAL) {
                        ...
                        pivot_shard_offset = set_intersection_initial_constraint(
                            dep,
                            num_query_vertices,
                            num_query_edges, 
                            match_vid,
                        ...
                        set_intersection_resume_constraint(
                            dep,
                            match_vid,
                            num_join_shards,
                            num_constraints_per_block,
