...
uint32_t set_intersection_initial_optional_constraint(
        uint32_t const& dep,
        ...
        bool const* is_optional_vertex, // for optional
        uint32_t* vid_gap,
    ...
    // }
    if (threadIdx.x == 0) {
        for (uint32_t i = 1; i < num_join_shards; i++) {
    ...
    if (!num_constraints_per_block) {
        if (vid_gap[0] == 0 || (dep != 0 && num_join_shards == 1 && !(dep != num_query_vertices - 1 && !is_optional_vertex[dep + 1] && vid_gap[0] >= SMThresholdOpt))) { // for optional
            if (threadIdx.x == 0) {
            ...
            uint32_t tmp;
            bool intersect;
            uint32_t max_value = min(max_iter_per_shard[dep][pivot_e_id], min_iter_per_shard[dep][pivot_e_id] + SMThresholdOpt - 1);
            ...
            __syncthreads();
            if (threadIdx.x == 0)
                min_iter_per_shard[dep][pivot_e_id] += SMThresholdOpt;

            return pivot_shard_offset;
        }
    }
    else { // for constraint
        if (vid_gap[0] == 0 || (dep != 0 && num_join_shards == 1 && dep != num_query_vertices - 1 && !(dep != num_query_vertices - 1 && !is_optional_vertex[dep + 1] && vid_gap[0] >= SMThresholdOpt))) { // for optional
            if (threadIdx.x == 0) {
                im_size[dep] = vid_gap[0];
                is_type[dep] = intersection_type::SINGLE;
            }
...
GSTREAM_DEVICE_ONLY ASH_FORCEINLINE
uint32_t set_intersection_resume_optional_constraint(
        uint32_t const& dep,
        uint32_t const& match_vid,
        uint32_t const& num_join_shards,
        uint32_t const& num_constraints_per_block, // for constraint
...
GSTREAM_CUDA_KERNEL
void LFTJ_kernel_optional_constraint(kernel_args const args) {
    ...
    __shared__ uint32_t sched_tree_prv_size[MaxNumTreeNodes - 1]; // for optional
    ...
    __shared__ uint32_t im_data[MaxNumQueryVertices][SMThresholdOpt];
    __shared__ bool is_optional_vertex[MaxNumQueryVertices]; // for optional
    __shared__ uint32_t num_optional_per_vertex[MaxNumQueryVertices]; // for optional
    __shared__ bool is_null_value[MaxNumQueryVertices][SMThresholdOpt]; // for optional
    ...
    for (uint32_t i = threadIdx.x; i <= num_query_vertices; i += blockDim.x) {
        if (i < num_query_vertices) {
            ...
            is_optional_vertex[i] = kbuf->is_optional_vertex[i];
            num_optional_per_vertex[i] = kbuf->num_optional_per_vertex[i];
        }
    ...
    for (uint32_t i = threadIdx.x; i < num_query_vertices; i += blockDim.x) {
        num_join_shards_arr[i] = prefix_sum_degree[i + 1] - prefix_sum_degree[i] - num_optional_per_vertex[i];
    ...
    for (uint32_t i = threadIdx.x + 1; i < num_sched_tree_nodes; i += blockDim.x) {
        uint32_t const tree_prv_id = sched_tree_prv_id[i - 1];
        sched_tree_prv_size[i] = sched_tree_ptr[tree_prv_id + 1] - sched_tree_ptr[tree_prv_id];
    }
    ...
    for (uint32_t st_idx = 0; st_idx < kbuf->num_merged_subtasks; st_idx++) {
        ...
        while (true) {
            ...
            if (dep == num_query_vertices - 1) {
                uint32_t const pivot_shard_offset = set_intersection_initial_optional_constraint(
                    dep,
                    num_query_vertices,
                    num_query_edges, 
                    match_vid,
                ...
                if (threadIdx.x == 0) {
                    if (im_size[dep] != 0) is_null_value[dep][im_size[dep - 1]] = false; // for optional
                    if (sched_tree_prv_size[tree_node_id] == sched_tree_child[dep - 1] + 1 && is_null_value[dep][im_size[dep - 1]]) { // for optional
                        im_size[dep] = 1;
                    //     // im_data[dep][0] = NULL_VALUE;
                    }

                    // if (blockIdx.x < 5 && threadIdx.x == 0) {
                    //     if (im_size[dep] == 1)
                    //         for (uint32_t i = 0; i < im_size[dep]; i++)
                    //             printf("%lu B: %u %u %u %u\n", thread_local_counter + i, rslt[0], rslt[1], NULL_VALUE, blockIdx.x);
                    //     else 
                    //         for (uint32_t i = 0; i < im_size[dep]; i++)
                    //             printf("%lu B: %u %u %u %u\n", thread_local_counter + i, rslt[0], rslt[1], 0, blockIdx.x);
                    // }
                ...
                if (is_finish[dep]) { 
                    if (state[dep] == state_type::INITIAL) {
                        ...
                        pivot_shard_offset = set_intersection_initial_optional_constraint(
                            dep,
                            num_query_vertices,
                            num_query_edges, 
                            match_vid,
                        ...
                        if (threadIdx.x == 0) {
                            ...
                            is_finish[dep] = false;
                            if (is_optional_vertex[dep] && im_size[dep] != 0) is_null_value[dep][im_size[dep - 1]] = false;  // for optional
                            if (is_type[dep] != intersection_type::SHARED_MEMBUF || min_iter_per_shard[dep][pivot_e_id] > max_iter_per_shard[dep][pivot_e_id]) {
                                ...
                                if (is_optional_vertex[dep] && is_null_value[dep][im_size[dep - 1]] && sched_tree_prv_size[tree_node_id] == sched_tree_child[dep - 1] + 1) {
                                    im_size[dep] = 1;
                                    im_data[dep][0] = NULL_VALUE;
                                    is_type[dep] = intersection_type::SHARED_MEMBUF;
                                }
                        ...
                        if (is_optional_vertex[dep + 1]) { // for optional
                            for (uint32_t i = threadIdx.x; i < im_size[dep]; i += blockDim.x) {
                                is_null_value[dep + 1][i] = true;
                            }
                        }
                    }
                    else { // resume intersection
                        set_intersection_resume_optional_constraint(
                            dep,
                            match_vid,
                            num_join_shards,
                            num_constraints_per_block,
