...
char* size2str_SI(uint64_t size, char* out, size_t bufsize);
char* size2str_IEC(uint64_t size, char* out, size_t bufsize);
char* size2str_IEC(uint64_t size, size_suffix base, char* out, size_t bufsize);
char* quantity2str_SI(uint64_t quantity, char* out, size_t bufsize);
...
std::string format_bytes_to_readable(uint64_t bytes);
std::pair<char*, char*> tokenize_dirname_basename(char* path);
} // !namespace ash
#endif // ASH_STRING_H
