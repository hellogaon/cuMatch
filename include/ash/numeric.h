...
ASH_FORCEINLINE constexpr T KB(uint64_t const i) {
    assert(i <= (std::numeric_limits<T>::max() / 1000llu)); // Check for overflow
    T result = i * 1000;
    assert(result >= i); // Check for overflow in result
    return result;
}

template <typename T = uint64_t>
ASH_FORCEINLINE constexpr T MB(uint64_t const i) {
    assert(i <= (std::numeric_limits<T>::max() / 1000000llu)); // Check for overflow
    T result = i * 1000000;
    assert(result >= i); // Check for overflow in result
    return result;
}
...
template <typename T = uint64_t>
ASH_FORCEINLINE constexpr T GB(uint64_t const i) {
    assert(i <= (std::numeric_limits<T>::max() / 1000000000llu)); // Check for overflow
    T result = i * 1000000000;
    assert(result >= i); // Check for overflow in result
    return result;
}

template <typename T = uint64_t>
ASH_FORCEINLINE constexpr T TB(uint64_t const i) {
    assert(i <= (std::numeric_limits<T>::max() / 1000000000000llu)); // Check for overflow
    T result = i * 1000000000000llu;
    assert(result >= i); // Check for overflow in result
    return result;
}

template <typename T = uint64_t>
ASH_FORCEINLINE constexpr T KiB(uint64_t const i) {
    assert(i <= (std::numeric_limits<T>::max() >> 10)); // Check for overflow
    T result = i << 10;
    assert(result >= i); // Check for overflow in result
    return result;
}
...
template <typename T = uint64_t>
ASH_FORCEINLINE constexpr T MiB(uint64_t const i) {
    assert(i <= (std::numeric_limits<T>::max() >> 20)); // Check for overflow
    T result = i << 20;
    assert(result >= i); // Check for overflow in result
    return result;
}
...
template <typename T = uint64_t>
ASH_FORCEINLINE constexpr T GiB(uint64_t const i) {
    assert(i <= (std::numeric_limits<T>::max() >> 30)); // Check for overflow
    T result = (T)(i << 30);
    assert(result >= i); // Check for overflow in result
    return result;
}
...
template <typename T = uint64_t>
ASH_FORCEINLINE constexpr T TiB(uint64_t const i) {
    assert(i <= (std::numeric_limits<T>::max() >> 40)); // Check for overflow
    T result = i << 40;
    assert(result >= i); // Check for overflow in result
    return result;
}
...
struct naturally_aliged_size {
    constexpr static uint64_t value = aligned_size(S, ASH_ARCH_ALIGN);
};

#define ASH_NATURALLY_ALIGNED_SIZE(S, Value) \
template <>\
struct naturally_aliged_size<S> {\
    constexpr static uint64_t value = Value;\
};
