...
#define GSTREAM_INVOKE_CUDA_KERNEL_2(Kernel, GridDim, BlockDim) Kernel
    //_ash_unused(GridDim); _ash_unused(BlockDim); Kernel
#define GSTREAM_INVOKE_CUDA_KERNEL_3(Kernel, GridDim, BlockDim, Shm) Kernel
    //_ash_unused(GridDim); _ash_unused(BlockDim); _ash_unused(Shm); Kernel
#define GSTREAM_INVOKE_CUDA_KERNEL_4(Kernel, GridDim, BlockDim, Shm, Stream) Kernel
    //_ash_unused(GridDim); _ash_unused(BlockDim); _ash_unused(Shm); _ash_unused(Stream); Kernel
#else
#define GSTREAM_INVOKE_CUDA_KERNEL_2(Kernel, GridDim, BlockDim) Kernel<<<GridDim, BlockDim>>>
#define GSTREAM_INVOKE_CUDA_KERNEL_3(Kernel, GridDim, BlockDim, Shm) Kernel<<<GridDim, BlockDim, Shm>>>
#define GSTREAM_INVOKE_CUDA_KERNEL_4(Kernel, GridDim, BlockDim, Shm, Stream) Kernel<<<GridDim, BlockDim, Shm, Stream>>>
#endif // !__INTELLISENSE__

#define __GSTREAM_INVOKE_CUDA_KERNEL_N(N) GSTREAM_INVOKE_CUDA_KERNEL_##N
#define _GSTREAM_INVOKE_CUDA_KERNEL_N(N) __GSTREAM_INVOKE_CUDA_KERNEL_N(N)
#define GSTREAM_INVOKE_CUDA_KERNEL(CUDA_Kernel, ...) \
    ASH_PP_EXPAND(_GSTREAM_INVOKE_CUDA_KERNEL_N(ASH_PP_NARG(__VA_ARGS__)) (CUDA_Kernel, __VA_ARGS__))
